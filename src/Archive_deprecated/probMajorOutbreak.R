...
#p major is the probability of a major outbreak with fraction p with high titre and values as in parameter list, and n introductions
pmajor <- function(param.list,p,n, prefer.low.titre = TRUE,...){
  dat<- data.frame(p = p)
  ...
  names(dat)<- c("p","q1","q2","Rv");
  if(prefer.low.titre)  {n1 <- ceiling(n*(1-p)); n2 <- floor(n*p)} else{n1 <- floor(n*(1-p)); n2 =ceiling(n*p)};
  dat$pmajor <- max(1-dat$q1^n1 * dat$q2^n2,0)
  return(dat)
}
