...
param.list <- list(
  ...
  p.protect = 1 - 6/26,#proportion initially protected by vaccination
  ...
  latency.period = c(0.00001,0.00001),#choose very short latency period
...
functions <- with(param.list, {list(
 dL = function(U,itype){return(-log(1 - U)*latency.period[itype])}, #exponential distribution#Duration latency period as function of a random variable U
