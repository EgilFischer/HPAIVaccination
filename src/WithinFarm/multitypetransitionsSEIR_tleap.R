...
gamma.waning.distribution <- function(t, param.list){
  ...
  with(param.list,{
    round((pgamma(t, shape = (trans.mean^2)/trans.var , scale = trans.var/trans.mean, lower.tail = FALSE)-(pgamma(t+deltat, shape = (trans.mean^2)/trans.var , scale = trans.var/trans.mean, lower.tail = FALSE)))/(pgamma(t, shape = (trans.mean^2)/trans.var , scale = trans.var/trans.mean, lower.tail = FALSE)),digits = 15)
