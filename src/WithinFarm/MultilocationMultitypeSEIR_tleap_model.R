...
param.list <- list(
  ...
  initial= c(10,1,0) , #initially infected
  ...
  alpha = 1,#parameter defining amount wihtin- and between transmission (alpha = 0 no between, alpha = 1 random mixing between locations)
  location_age = c(0,100,200),#ages of animals start of simulation
...
inits.gamma.buildup.wane <-  with(param.list,{
  ...
  p.protect.adjust <- p.protect*(pgamma(intro.time+location_age, shape = shape_buildup , scale = scale_buildup, lower.tail = TRUE));
  
  #adjust p.protect with waning
  shape_wane = (trans.mean.wane^2)/trans.var.wane;
  scale_wane =trans.var.wane/trans.mean.wane;
  p.protect.adjust <- p.protect.adjust*pgamma(intro.time+location_age, shape = shape_wane , scale = scale_wane, lower.tail = FALSE);
  
  return(list(
    L0 = round(c(initial, initial)*c(1-p.protect.adjust,p.protect.adjust),digits = 0), #number of initially latently infected 
    ...
    S0 = round(c(1-p.protect.adjust,p.protect.adjust)*(c(initial,initial)),digits = 0))#initially susceptible
