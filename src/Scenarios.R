...
print("Baseline")
print(Sys.time())
simulate.multitypeSIR(param.list.baseline.layer )
...
for(i in c(1:2)){
  ...
  param.list$p.hightitre <- c(0,0.5)[i]
  param.list$pdie <- c(0.5,0.005)
  param.list$scenario <- paste0("layerClinic50Phigh",param.list$p.hightitre*100);
...
for(i in c(1:2)){
  ...
  param.list$p.hightitre <- c(0,0.5)[i]
  param.list$pdie <- c(0.5,0.005)
  param.list$scenario <- paste0("broilerClinic50Phigh",param.list$p.hightitre*100);
  scenario.list.clinic[[i]]<- param.list
}
